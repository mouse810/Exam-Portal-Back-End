...
module.exports.getAllStudents = async function (req) {
  try {
...
module.exports.addStudent = async function (req) {
  try {
    ...
    if (!user || user.userType != APP_CONSTANTS.ACCOUNT_TYPE.STUDENT) return {
      status: statusCodes.NOT_FOUND,
...
module.exports.createStudent = async function (req) {
  try {
    let payload = req.body;
    let course = await Model.courses.findById(payload.courseID);
...
module.exports.deleteSubject = async function (req) {
  try {
...
module.exports.deleteCourse = async function (req) {
  try {
    let payload = req.params;
    let examiner = req.loggedUser;
    let fieldsToUpdate = { isDeleted: true };
...
module.exports.deleteStudent = async function (req) {
  try {
    let payload = req.params;
    let examiner = req.loggedUser;
    let fieldsToUpdate = {
...
module.exports.deleteQuestion = async function (req) {
  try {
    let payload = req.params;
    let examiner = req.loggedUser;
    let fieldsToUpdate = {
    ...
    if ((exam.examDate.getTime() + (exam.startTime.split(":")[0] * 3600000) + (exam.startTime.split(":")[1] * 60000) <= Date.now()))
      return {
        status: statusCodes.BAD_REQUEST,
        message: messages.EXAMINER_CANNOT_DELETED_QUESTION_ON_EXAMDATE
      };
    ...
    await Model.questions.findByIdAndUpdate(question._id, fieldsToUpdate);
    return {
      status: statusCodes.SUCCESS,
      message: messages.QUESTION_DELETED_SUCCESSFULLY
    }
  } catch (error) {
    throw error;
  }
...
module.exports.removeExamStudent = async function (req) {
  try {
    let payload = req.body;
    let examiner = req.loggedUser;
    let fieldsToUpdate = {
    ...
    let student = await Model.students.findById(payload.studentID);
    let exam = await Model.exams.findById(payload.examID);
    if (!student || student.isDeleted || !examStudent) return {
      status: statusCodes.NOT_FOUND,
      message: messages.STUDENT_NOT_FOUND
    }
    if (!exam || exam.examinerID != examiner.id || exam.isDeleted)
...
module.exports.updateExam = async function (req) {
  try {
    let payload = req.body;
    let examiner = req.loggedUser;
    let fieldsToUpdate = {
...
module.exports.updateCourse = async function (req) {
  try {
    let payload = req.body;
    let examiner = req.loggedUser;
    let fieldsToUpdate = {
